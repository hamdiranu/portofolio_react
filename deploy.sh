...
sudo docker build -f "Dockerfile" -t hamdiranu/containerd:frontend-$TRAVIS_BUILD_NUMBER  --label "Maintaner  Hamdi R <hamdi@alterra.id>" .
# push apps image to docker hub
sudo docker push hamdiranu/containerd:frontend-$TRAVIS_BUILD_NUMBER
...
kubectl -n alta9 --kubeconfig kubeconfig set image deployment/${APP_K8S_DEPLOYMENT_NAME} ${APP_K8S_CONTAINER_NAME}=hamdiranu/containerd:frontend-${TRAVIS_BUILD_NUMBER}
